...
export function BottomChat({ onIncomeAdded, onExpenseAdded, onBillPaid, fixedExpenses = [], currentNeeds, currentWants, currentSavings, totalBalance, monthlyIncome = 0, userName = '' }: Props) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemPrompt = 'Eres el asistente de finanzas de TomyGuia. Analiza el mensaje y responde SOLO con JSON valido sin markdown. Formato exacto: {"intent":"expense|income|bill|query|unknown","amount":0,"category":"needs|wants","label":"texto","feedback":"texto"} Contexto: ' + contexto + ' Reglas: intent expense=gasto, income=cobro, bill=recibo fijo, query=consulta saldo, unknown=otro. category needs=renta/luz/agua/internet/gas/medicamentos/transporte/super/despensa. wants=cafe/restaurante/antojo/snack/ropa/salidas/entretenimiento/lujos/papas/dulces/refrescos. Si es comida basica del super es needs, si es antojo o restaurante es wants. label=1-3 palabras. feedback=frase corta sin emojis max 50 chars.';
